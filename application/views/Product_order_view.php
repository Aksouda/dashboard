                        ...
                        <div class="portlet light bordered">
                            <div class="portlet-title">
                                <div class="caption font-dark">
                                    ...
                                    <span class="caption-subject bold uppercase"> Manage Orders</span>
                                </div>
                            </div>
                            <div class="portlet-body">
                                ...
                                <table class="table table-striped table-bordered table-hover table-checkable order-column" id="order">
                                    <thead>
                                    <tr>
                                        <th>
...
<!-- Modal -->
<button type="button" class="hide" data-backdrop="static" data-keyboard="false" data-toggle="modal" id="process_data_model" data-target="#myModal">Open Modal</button>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            <div class="modal-header label-primary">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title ">Process Order Success</h4>
            </div>
            <div id="now_click" class="modal-body">
                Orders from buyer 'Rocky Spero' successfully processed
            </div>
            <div class="modal-footer btn">
...
<script type="text/javascript" language="javascript" >
    $(document).ready(function() {
        ...
        $('#order').DataTable({
            "iDisplayLength": 10,
            ajax: '<?php echo base_url('Product_order/get_data'); ?>',
            columns: [
                { data: "cheked"},
                { data: "created_at" },
                { data: "user_name" },
    ...
    function delete_review(id) {
        swal({
            ...
            function(){
                $.ajax({
                    'url':'<?php echo base_url('Product_order/delete'); ?>',
                    'method':'POST',
                    'data':{'id':id},
                    'success':function(){
                        var table_review = $('#order').DataTable();
    ...
    $('#multi_delete').click(function (){
        ...
        swal({
            ...
            function () {
                swal("Deleted!", "Your imaginary file has been deleted.", "success");
                $.ajax({
                    'url':'<?php echo base_url('Product_order/multi_delete'); ?>',
                    'data':{'id':abc},
                    'method':'post',
    ...
    function process_Work(id) {
        swal({
                title: "Process Order Confirmation",
                text: "After processed, order status can not be changed and product stock will be reduced.Please review order before press button PROCESS ORDER",
                type: "info",
                showCancelButton: true,
                closeOnConfirm: true,
            ...
            function(){
            $.ajax({
                'url':'<?php echo base_url('Product_order/get_sep_product_order');?>',
                'method':'post',
                'data':{'id':id },
                'success':function (data) {
                    $('.modal-body').html(data);
                    $('#process_data_model').trigger('click');
                }
            });
        });
    }
    function place_one(id) {
        $.ajax({
            'url':'<?php echo base_url('Product_order/update_order_proccess'); ?>',
            'method':'post',
            'data':{'id':id},
            'success':function (data) {
                var table_review = $('#order').DataTable();
                table_review.ajax.reload(null,false);
                var new_data = $('#cartlist').html();
                $('#now_click').html(new_data);
                $('#process_success').trigger('click');
            }
        });
    }
    function cencel_my_product(id,name) {
        swal({
                title: "Cancel Order Confirmation ?",
                text: "Are you sure want to CANCEL order from : "+name+" ?",
                type: "warning",
                showCancelButton: true,
                confirmButtonColor: "#DD6B55",
            ...
            function(isConfirm){
                if (isConfirm) {
                    $.ajax({
                        'url':'<?php echo base_url('Product_order/cancel_my_order'); ?>',
                        'method':'post',
                        'data':{'id':id},
                        'success':function (data) {
                            var table_review = $('#order').DataTable();
                            table_review.ajax.reload(null,false);
                            swal("Deleted!", "This Order is Cancel.", "success");
                        }
                    });
                } else {
